...
public class LadderClone {
	...
	public LadderClone(int numberOfUsers) { // 사다리의 길이 10 고정, 인원수 만큼 생성
		super();
...
//  	for (int i = 0; i < NumberOfUsersArr.length; i++) { // 이름 저장 잘 되고 있나 확인중
//     	System.out.print(NumberOfUsersArr[i]);
// 	 	}
// 
	}
	...
	private void showOrNotShow() { // 사다리에 이름을 표시할지 말지 정하는 메소드
		...
		if (choose.equals("1") || choose.equals("1번") 
				|| choose.equals("1번 Yes") || choose.equals("YES")
				|| choose.equals("yes") || choose.equals("Yes") 
				|| choose.equals("1번 : Yes") 
				|| choose.equals("1번 : YES")) { // 사다리 첫번째 라인에 이름 표현
			...
			for (int i = 0; i < NumberOfUsersArr.length * 2; i++) {
				...
				|| choose.equals("2번 : No") || choose.equals("2번 : NO")) { // 사다리
																															// 표현
...
//		}
		...
		for (int i = 0; i < ladder[0].length; i++) { // 사다리 마지막 부분에 저장
				...
				}else if (i % 2 != 0 && i > 6) { // 당첨여부 라인 공백 맞추기용
